...
void CodePublisher::createBody(std::ofstream& out, std::string fileName, std::string htmlLoc, DependencyTable depTable, std::ifstream& htmlFile) {
	std::string line;
	out << "<!-" << "\n";
...
void CodePublisher::doAnalysis(int argc, char* argv[], DependencyTable& depTable, TypeTable& typeTable, CodeAnalysis::CodeAnalysisExecutive& exec) {
...
void CodePublisher::displayPublisher(std::vector<std::string> files, int argc, char* argv[], std::string WebPageRepositoryLoc) {
...
std::string CodePublisher::getWebPageRepo(int argc, char* argv[]) {
	...
	WebPageRepositoryLoc += currentDateTime() + "\\";
...
int main(int argc, char* argv[]) {
	...
	std::cout << "\n\n\n================================================================================================================================================";
	cp.displayPattern(8);
	Path WebPageRepositoryLoc = cp.getWebPageRepo(argc, argv);
	if (WebPageRepositoryLoc.compare("invalid") == 0) {
		std::cout << "Invalid Repository to Store Files\n";
		return 1;
	}
	cp.doAnalysis(argc, argv, depTable, typeTable, exec); // Type Table creation + Dependency Analysis
	...
	cp.publishFiles(filesCPP, list_of_HtmlFiles, WebPageRepositoryLoc, depTable, typeTable);
	cp.displayRequirements();
	cp.displayPublisher(list_of_HtmlFiles, argc, argv, WebPageRepositoryLoc);
	std::cin.get();
}