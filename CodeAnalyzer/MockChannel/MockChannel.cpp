...
{
	std::cout << "\n\n =================>>>>>> Mock channel received msg From GUI: ==>>> \n  " << msg << "\n";
	sendQ_.enQ(msg);
}
...
void MockChannel::start()
{
	...
	thread_ = std::thread(
		...
		while (!stop_)
		{
			...
			recvQ.enQ(msg);
			std::vector<std::string> msgTypeVec = Utilities::StringHelper::split(msg);
			std::string type = msgTypeVec[msgTypeVec.size() - 1];
			if (type == "GetFilesNoParentList" || type == "DisplayFile" || type == "IISFiles" ||type == "LazyDownload" ||type == "PublishFiles" || type == "SendFiles" || type == "DownloadFiles" || type == "GetFilesList" || type == "deleteSelectedFiles") {
				std::thread t1([&]() {
					std::cout << "\n\n\n\n\t\t ==================Starting HttpMessage client on thread===============================\n\n";
					std::string toGui = c1.execute1(msg);
					if (toGui.length() > 0) {
						std::cout << "\n ================>>>>>  Message Received from server to Poston GUI: \n  " << toGui <<  "\n";
						pSendr->postMessage(toGui);
					}
				});
				t1.join();
			}
