...
bool CodeAnalysisExecutive::checkPath(std::string path_) {
...
bool CodeAnalysisExecutive::displayArg(Patterns patterns_) {
	if (patterns_.size() == 0)
	{
		showUsage(); 
		return false;
	...
	try {
		...
		for (int i = 2; i < argc; ++i)
		{
			...
			if (argument == "/dp")
			{
				...
				{
					xmlPath = argument; std::cout << "\n  Dependency File XML will be created with Name: --> " << argument; 
					checkXML = true;
				}
			}
...
// For Project#3 this executive is acting as a stub to implement Project #2 functionality.
#ifdef CodeAnalyzer
